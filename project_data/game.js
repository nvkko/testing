...
var arr1 = ['firstTxt', 'secondTxt', 'thirdTxt'], txt_i = 0, timer = 0, step = 0, step2 = 0;
const G = {
  state:"main", sound:true, needResize:true
  ,onload(){
    c = ctx;
    c.textAlign = "center"; c.textBaseline = "middle"; c.font = "bold "+data.fontSize+"px font1"; c.fillStyle = "#fff";
  ...
  ,loop(){
    ...
    c.fillStyle = '#000'; c.resetTransform(); c.fillRect(0, 0, cw, ch);
...
function draw_step0(text) {
  ...
  if(timer%20 > 5) c.fillText('_', x, y);
  c.fillText(' ', x, y+line);
...
function draw_step2(text) {
  ...
  if(timer%20 > 5) c.fillText(text[i]+'_', x, y+line);
...
function draw_link(text) {
  ...
  if(timer > 10) c.fillText(text[i]+'_', x, y+line);
  c.fillText(text[i]+' ', x, y+line);
